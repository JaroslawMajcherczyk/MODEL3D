...
// main.jsx (lub plik startowy, który pokazałeś)
import { StrictMode } from 'react';
import { createRoot } from 'react-dom/client';
import './index.css';
import App from './App.jsx';
...
(function setup() {
  ...
  // helper: ogłoś gotowość całości do C#
  function maybeAnnounceReady() {
    if (ready.signalr && ready.viewer && ready.model) {
      window.Nexus.send?.('ready', { ts: Date.now() });
      console.log('[Nexus] READY announced');
    }
  }

  // helper: wyślij info, że actions gotowe (gdy mamy już send)
  function announceActionsReadyIfPossible() {
    if (window.Nexus?.actions && window.Nexus?.send) {
      try { window.Nexus.send('ActionsReady'); } catch { /* noop */ }
    }
  }

  // kiedy actions się zarejestrują (modelActions.jsx emituje event)
  window.addEventListener('nexus:actions:ready', () => {
    announceActionsReadyIfPossible();
  });
  ...
  // --- Tryb DEV: bez backendu (env lub ?nohub=1 w URL) ---
  const sp = new URLSearchParams(location.search);
  const noHubParam = sp.has('nohub'); // ?nohub=1
  const DEV_NO_BACKEND =
    (import.meta.env?.VITE_DEV_NO_BACKEND === '1') || noHubParam;

  // adres huba (jeśli jednak łączymy się do backendu)
  const hubParam = sp.get('hub'); // np. ?hub=http://127.0.0.1:5183/nexus
  const url =
    hubParam ||
    `${location.protocol}//${location.hostname}:5177/nexus` || // dev domyślny
    `${location.origin}/nexus`; // fallback reverse-proxy / prod

  // HMR guard
  if (window.__NEXUS_CONN__) return;

  // ————————————————————————————————————————————
  // GAŁĄŹ BEZ BACKENDU (dev/offline)
  // ————————————————————————————————————————————
  if (DEV_NO_BACKEND) {
    console.warn('[Nexus] DEV_NO_BACKEND mode enabled – skipping SignalR');
    window.Nexus.send = (evt, payload) => {
      console.log('[Nexus:send:DEV]', evt, payload ?? null);
      return Promise.resolve();
    };
    ready.signalr = true; // odhaczone – nie czekamy na serwer
    announceActionsReadyIfPossible(); // jeśli actions już są
    maybeAnnounceReady();             // jeśli viewer + model już gotowe
    return;
  }

  // ————————————————————————————————————————————
  // NORMALNE POŁĄCZENIE PRZEZ SIGNALR
  // ————————————————————————————————————————————
  const conn = new HubConnectionBuilder()
    .withUrl(url)
    .withAutomaticReconnect()
